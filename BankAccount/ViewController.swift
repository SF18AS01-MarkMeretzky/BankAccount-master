...
class ViewController: UIViewController {
    ...
    @IBAction func depositButtonPressed(_ sender: UIButton) {
        if let text: String = textField.text {
            if let amount: Double = Double(text) {
                if amount < 0 {
                    label.text = "deposit cannot be negative"
                } else {
                    sender.resignFirstResponder();
                    balance += amount;
                    label.text = String(format: "You just deposited: $%.2f, new balance is $%.2f", amount, balance);
                }
            }
        }
    }
    ...
    @IBAction func balanceButtonPressed(_ sender: UIButton) {
        ...
        label.text = String(format: "Your balance is: $%.2f", balance);
    }
